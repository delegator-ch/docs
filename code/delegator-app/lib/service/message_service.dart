...
class MessageService {
  ...
  // Fetch all messages with auto-retry
  ...
  // Fetch messages for a specific chat with auto-retry
  Future<List<Message>> fetchMessagesByChat(int chatId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (tokenFetched) {
          // Try again with the new token
          return fetchMessagesByChat(chatId);
        } else {
          // Still failed
          TokenManager.clearToken();
          throw Exception('Authentication failed. Please login again.');
      ...
      print('Error in fetchMessagesByChat: $e');
      throw e;
    }
  }

  // Send a new message with auto-retry
